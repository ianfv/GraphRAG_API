...
class TextChunker:
    def __init__(
        self, chunk_size: int = 1200, chunk_overlap: int = 100, encoding_name: str = "cl100k_base"
    ):
    ...
    def chunk_by_sentences(self, text: str, doc_id: str) -> list[Chunk]:
        ...
        for sentence in sentences:
            ...
            if len(current_tokens) + sent_len > self.chunk_size:
                ...
                current_start_idx += (
                    len(chunks[-1].text) if chunks else 0
                )  # Logic simplified for stream
    ...
    def chunk_group_strings(
        self, text_parts: list[str], doc_id: str, metadata: dict
    ) -> list[Chunk]:
        """
